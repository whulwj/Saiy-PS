...
public class UtilsWearMessage {
    ...
    private static void sendMessage(Context context, String wearContent, WearMessageEvent wearMessageEvent) {
        if (DEBUG) {
        ...
        intent.putExtra(LocalRequest.EXTRA_WEAR, wearContent);
        intent.putExtra(WearListenerService.EXTRA_WEAR_MESSAGE, wearMessageEvent);
        context.getApplicationContext().startService(intent);
    }

    public static void sendMessage(Context context, String eventMessage, String wearContent, Locale eventLocale, @WearMessageEvent.Type byte eventType, @Nullable Bundle bundle) {
        if (DEBUG) {
            MyLog.i(CLS_NAME, "prepareMessage");
        }
        if (!UtilsBundle.notNaked(bundle)) {
            sendMessage(context, wearContent, new WearMessageEvent(eventType, UtilsString.convertProperCase(eventMessage, eventLocale), UtilsLocale.convertToIETF(eventLocale), null));
            return;
        }
        ...
        sendMessage(context, wearContent, new WearMessageEvent(eventType, UtilsString.convertProperCase(eventMessage, eventLocale), UtilsLocale.convertToIETF(eventLocale), intent.toUri(Intent.URI_INTENT_SCHEME)));
    }
}
