...
public class TranslationProviderReference {
    ...
    private static final String CLS_NAME = TranslationProviderReference.class.getSimpleName();
    ...
    public @Nullable TranslationProvider getRequestTranslation() {
        ...
        com.google.firebase.database.FirebaseDatabase.getInstance().getReference("db_read").child("provider").child("translation").addListenerForSingleValueEvent(new com.google.firebase.database.ValueEventListener() {
            ...
            public void onDataChange(@NonNull com.google.firebase.database.DataSnapshot dataSnapshot) {
                if (DEBUG) {
                    MyLog.i(CLS_NAME, "getRequestTranslationProvider: onDataChange");
                }
                taskCompletionSource.setResult(dataSnapshot.getValue(TranslationProvider.class));
            }

            @Override
            public void onCancelled(@NonNull com.google.firebase.database.DatabaseError error) {
                if (DEBUG) {
                    MyLog.i(CLS_NAME, "getRequestTranslationProvider: onCancelled");
                }
        ...
        try {
            ...
            if (DEBUG) {
                MyLog.w(CLS_NAME, "getRequestTranslationProvider: InterruptedException");
                e.printStackTrace();
            }
        } catch (ExecutionException e) {
            if (DEBUG) {
                MyLog.w(CLS_NAME, "getRequestTranslationProvider: ExecutionException");
                e.printStackTrace();
            }
        } catch (TimeoutException e) {
            if (DEBUG) {
                MyLog.w(CLS_NAME, "getRequestTranslationProvider: TimeoutException");
                e.printStackTrace();
            }
        } catch (Exception e) {
            if (DEBUG) {
                MyLog.w(CLS_NAME, "getRequestTranslationProvider: Exception");
                e.printStackTrace();
            }
        }
        return null;
